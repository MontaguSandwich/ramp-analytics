...
if [ "$BRANCH_CODE" = "404" ]; then
  ...
  # A missing snapshot here is a real failure and fails the build.
  #
  # Adding a NEW venue is a two-step dance, because an adapter ships in the same commit
  # that first deploys it — at that moment the cron has never run with it, so its snapshot
  # genuinely doesn't exist on the data branch and a strict 404 would make the venue
  # undeployable (build fails → old deployment without it keeps serving → the cron never
  # gets a chance to matter). Ship the venue with its id in a temporary loop that tolerates
  # 404 (see git history for 2026-07-22 revolut), let one cron cycle populate the data
  # branch, then move the id here. Both Revolut venues completed that dance on 2026-07-22.
  for id in zkp2p binance_p2p ramp_network revolut_ramp revolut; do
