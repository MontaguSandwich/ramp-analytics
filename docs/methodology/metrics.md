...
**Both dollar stablecoins are counted.** A venue's dollar liquidity is the sum of its USDT *and* USDC books, not whichever ticker is largest. This matters more than it sounds: after Binance delisted USDT for EEA users under MiCA in 2025, European liquidity migrated to USDC — measured within the ±5% band, USDC adds ~3% to the USD market but ~38% to EUR. Counting only USDT would have understated European depth by about a quarter. We include stablecoins only, since each is ≈$1 and sums into dollars without an FX step; volatile assets answer a different question and are not added in.
